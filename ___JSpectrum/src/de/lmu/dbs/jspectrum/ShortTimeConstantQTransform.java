 ...
 *
 */
public class ShortTimeConstantQTransform extends ShortTimeTransform {
	 ...
	 */
	private ConstantQTransform cqt = null;
	 ...
	 * @throws Exception
	 */
	public ShortTimeConstantQTransform(final double sampleRate, final double fmin, final double fmax, final double binsPerOctave) throws Exception {
	 ...
	 * @throws Exception
	 */
	public ShortTimeConstantQTransform(final double sampleRate, final double fmin, final double fmax, final double binsPerOctave, final double threshold, final double spread) throws Exception {
	 ...
	 * @throws Exception
	 */
	public ShortTimeConstantQTransform(final double sampleRate, final double fmin, final double fmax, final double binsPerOctave, final double threshold, final double spread, final double divideFFT) throws Exception {
	 ...
	 * @throws Exception
	 */
	public ShortTimeConstantQTransform(final double sampleRate, final double fmin, final double fmax, final double binsPerOctave, final double threshold, final double spread, final double divideFFT, final String kernelBufferLocation) throws Exception {
		cqt = new ConstantQTransform(sampleRate, fmin, fmax, binsPerOctave, threshold, spread, divideFFT, kernelBufferLocation);
		windowSize = cqt.getWindowSize();
		bins = cqt.getNumberOfOutputBands();
	}
	...
	public double[][] calculate(final int[] samples, final int step, final Window windowFunction) {
		...
		for(int frame=0; frame<frames; frame++) {
			...
			cqt.calcMagnitude(frameBuffer, data[frame]);
		}
		return data;
	}
