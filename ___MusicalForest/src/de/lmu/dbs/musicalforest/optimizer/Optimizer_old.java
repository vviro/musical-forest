...
public class Optimizer_old {
	 ...
	 */
	public Optimizer_old(int granularity, int testRadiusX, int testRadiusY, int meanShiftWindow, ForestMeta meta) {
	...
	public ForestMeta optimize(Forest2d forest, List<Dataset> datasets, int numOfClassifyingThreads) throws Exception {
		...
		return null; //new ForestMeta(bestOnsetThreshold, bestOnsetTest, bestOffsetThreshold, bestOffsetTest, null);
	...
	public void processThreaded(Worker worker, Forest2d forest, TreeDataset2d dataset, float[][][] classification, AccuracyTest[][] testsOnset, AccuracyTest[][] testsOffset, int num) throws Exception {
		...
		for(int i=0; i<granularity; i++) {
			...
			for(int j=0; j<granularity; j++) {
			    ...
			    byte[][] midi = newMidi.toDataArray(dataForestOnset.length, (int)meta.dataMeta.midiOffset, duration, meta.dataMeta.transformParams.frequencies);
