 ...
 *
 */
public class FeatureKinect4 extends Feature {
	 ...
	 * 
	 */
	public FeatureKinect4(int xMin, int xMax, int yMin, int yMax, int thresholdMax) {
	 ...
	 * 
	 */
	public FeatureKinect4(FeatureKinect4 f, int thresholdMax) {
		this.uX = f.uX;
		this.uY = f.uY;
		this.vX = f.vX;
		this.vY = f.vY;
	...
	public static List<Feature> getRandomFeatureSet(RandomTreeParameters params) {
		...
		for(int i=0; i<params.numOfRandomFeatures; i++) {
			FeatureKinect4 n = new FeatureKinect4(params.xMin, params.xMax, params.yMin, params.yMax, params.thresholdMax);
			ret.add(n);
			for(int j=0; j<params.thresholdCandidatesPerFeature-1; j++) {
				ret.add(new FeatureKinect4(n, params.thresholdMax));
			}
		}
		return ret;
	...
	public int evaluate(final byte[][] data, final int x, final int y) throws Exception {
		...
		return data[x][y]/3 + data[iuX][iuY] - data[ivX][ivY];
	}
}