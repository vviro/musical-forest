 ...
 *
 */
public class FeatureHarmonic4 extends Feature2d {
	 ...
	 * 
	 */
	public FeatureHarmonic4(final ForestParameters params) {
	 ...
	 * 
	 */
	public FeatureHarmonic4(final FeatureHarmonic4 f) {
		this.uX = f.uX;
		this.uY = f.uY;
		this.vX = f.vX;
		this.vY = f.vY;
	...
	public List<Feature2d> getRandomFeatureSet(ForestParameters params) {
		...
		for(int i=0; i<params.numOfRandomFeatures; i++) {
			FeatureHarmonic4 n = new FeatureHarmonic4(params);
			ret.add(n);
			for(int j=0; j<params.thresholdCandidatesPerFeature-1; j++) {
				ret.add(new FeatureHarmonic4(n));
			}
		}
		return ret;
	...
	public float evaluate(final byte[][] data, final int x, final int y) throws Exception {
		...
		if (u && v) return (data[iuX][iuY] + data[ivX][ivY])*data[x][y] + data[x][y]*data[x][y]*2; 
		if (u) return data[iuX][iuY]*data[x][y] + data[x][y]*data[x][y]*2;
		if (v) return data[ivX][ivY]*data[x][y] + data[x][y]*data[x][y]*2;
