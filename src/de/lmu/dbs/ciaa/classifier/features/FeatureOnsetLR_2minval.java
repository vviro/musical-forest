 ...
 *
 */
public class FeatureOnsetLR_2minval extends Feature2d {
	 ...
	 * 
	 */
	public FeatureOnsetLR_2minval(final ForestParameters params) {
		initStatic();
	...
	public List<Object> getRandomFeatureSet(ForestParameters params) {
		...
		for(int i=0; i<params.numOfRandomFeatures; i++) {
			FeatureOnsetLR_2minval n = new FeatureOnsetLR_2minval(params);
			ret.add(n);
		}
		return ret;
	}
	...
	public float evaluate(final byte[][] data, final int x, final int y) throws Exception {
		...
		for(int j=0; j<chosenHarmonics.length; j++) {
			...
			ret+= (data[x][ny] - data[x-uX][ny]) * d2 * data[x+vX][ny] * harmonicFactors[j];
		}
		return ret;
	}
	...
	public Feature2d getInstance(ForestParameters params) {
		return new FeatureOnsetLR_2minval(params);
	}
}