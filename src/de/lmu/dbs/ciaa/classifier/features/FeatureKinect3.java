 ...
 *
 */
public class FeatureKinect3 extends Feature {
	 ...
	 * 
	 */
	public FeatureKinect3(int xMin, int xMax, int yMin, int yMax, int thresholdMax) {
	 ...
	 * 
	 */
	public FeatureKinect3(FeatureKinect3 f, int thresholdMax) {
		this.uX = f.uX;
		this.uY = f.uY;
		this.vX = f.vX;
		this.vY = f.vY;
	...
	public static List<Feature> getRandomFeatureSet(RandomTreeParameters params) {
		...
		for(int i=0; i<params.numOfRandomFeatures; i++) {
			FeatureKinect3 n = new FeatureKinect3(params.xMin, params.xMax, params.yMin, params.yMax, params.thresholdMax);
			ret.add(n);
			for(int j=0; j<params.thresholdCandidatesPerFeature-1; j++) {
				ret.add(new FeatureKinect3(n, params.thresholdMax));
			}
		}
		return ret;
