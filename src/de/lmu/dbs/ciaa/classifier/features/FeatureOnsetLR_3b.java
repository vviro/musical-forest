 ...
 *
 */
public class FeatureOnsetLR_3b extends Feature2d {
	 ...
	 * 
	 */
	public FeatureOnsetLR_3b(final ForestParameters params) {
		initStatic();
	...
	private void generateXDeviation(int height) {
		...
		for(int y=0; y<height; y++) {
			xDeviation[y] = (int)(Math.pow(1.5, (float)(height - y) / 48.0) / 1.0) - 1;
		}
	}
	...
	public List<Object> getRandomFeatureSet(ForestParameters params) {
		...
		for(int i=0; i<params.numOfRandomFeatures; i++) {
			FeatureOnsetLR_3b n = new FeatureOnsetLR_3b(params);
			ret.add(n);
		}
		return ret;
	}
	...
	public float evaluate(final byte[][] data, final int x, final int y) throws Exception {
		...
		for(int j=0; j<chosenHarmonics.length; j++) {
			...
			ret+= (data[x-xOffset][ny] - data[x-xOffset-uX][ny]) * d2 * data[x+vX][ny] * harmonicFactors[j];
		}
		return ret;
	}
	...
	public Feature2d getInstance(ForestParameters params) {
		return new FeatureOnsetLR_3b(params);
	}
}