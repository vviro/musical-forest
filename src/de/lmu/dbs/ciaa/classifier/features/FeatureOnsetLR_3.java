 ...
 *
 */
public class FeatureOnsetLR_3 extends Feature2d {
	 ...
	 * 
	 */
	public FeatureOnsetLR_3(final ForestParameters params) {
		initStatic();
	...
	public List<Object> getRandomFeatureSet(ForestParameters params) {
		...
		for(int i=0; i<params.numOfRandomFeatures; i++) {
			FeatureOnsetLR_3 n = new FeatureOnsetLR_3(params);
			ret.add(n);
		}
		return ret;
	}
	...
	public float evaluate(final byte[][] data, final int x, final int y) throws Exception {
		...
		for(int j=0; j<chosenHarmonics.length; j++) {
			...
			ret+= (data[x-xOffset][ny] - data[x-xOffset-uX][ny]) * d2 * data[x-xOffset+vX][ny] * harmonicFactors[j];
		}
		return ret;
	}
	...
	public Feature2d getInstance(ForestParameters params) {
		return new FeatureOnsetLR_3(params);
	}
}